...
export const pdfGenerator = async ({ Header, farmerName, farmerVillage, today, udharEntries, jamaEntries, udharSum, jamaSum }) => {
    const udharEntriesHtml = udharEntries.map(entry => `
        <tr style="width:100%">
            <td style="">${entry.RUPEE}</td>
            <td>${entry.DATE}</td>
            <td style="text-align:right;width: 50%;">${entry.DETAILS}</td>
        </tr>
    `).join('');
    const jamaEntriesHtml = jamaEntries.map(entry => `
        <tr style="width:100%">
            <td>${entry.RUPEE}</td>
            <td>${entry.DATE}</td>
            <td style=" text-align:right;padding-right:10px;width: 50%;">${entry.DETAILS}</td>
        </tr>
    `).join('');
    const htmlContent = `
        <!DOCTYPE html>
